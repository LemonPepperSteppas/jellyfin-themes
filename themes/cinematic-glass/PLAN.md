...
Ship the **variable** woff2 (`wght` 200–800), not four statics — one file per subset,
smaller than the four weights the spec calls for, covering 400/500/700/800 from one
`font-weight: 200 800` declaration.

**Done.** `assets/manrope-latin.woff2` (24.8 KB) and `assets/manrope-latin-ext.woff2`
(15.1 KB), Manrope v20 from fonts.gstatic.com, committed with `assets/OFL.txt`.
`src/03-fonts.css` is generated by `build/embed-font.py`; `--check` verifies it is in step
with the assets. latin-ext is included because a media library carries European titles and
missing glyphs would silently drop to the fallback mid-word.
...
- [x] `03-fonts.css` — the embedded Manrope `@font-face` (see section 3a); GENERATED
